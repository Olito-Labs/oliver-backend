...
async def stream_fdl_ingest_text(payload: Dict[str, Any], user=Depends(get_current_user)):
    """Stream FDL ingestion for raw text with real-time reasoning.

    Mirrors /fdl/ingest but accepts raw text instead of a stored document.
    Emits identical SSE event types so the frontend UI can be reused.
    """
    document_text = payload.get('text') or ''
    study_id = payload.get('study_id')

    if not document_text.strip() or not study_id:
    ...
    async def generate_stream() -> AsyncGenerator[str, None]:
        try:
            ...
            # Step 1: Receive Text
            step1 = ReasoningStep(
                "Receiving First Day Letter Text",
                "Text input received. Preparing for analysis...",
                "file"
            )
            yield await send_reasoning_step(step1)
            await asyncio.sleep(0.5)
            ...
            if text_length < 50:
                step1.error("Provided text appears too short for analysis")
                yield await send_reasoning_step(step1)
                yield await send_error("Text too short for analysis")
                return
            step1.complete()
            step1.content = f"Received ~{text_length} characters of FDL text."
            yield await send_reasoning_step(step1)
            ...
            client = openai_manager.get_client()
            if not client:
                step2.error("OpenAI client not available")
                yield await send_reasoning_step(step2)
                yield await send_error("AI analysis service unavailable")
                return
            step2.complete()
            step2.content = f"Model {settings.OPENAI_EXAM_MODEL or settings.OPENAI_MODEL} connected successfully for text analysis."
            yield await send_reasoning_step(step2)

            exam_model = settings.OPENAI_EXAM_MODEL or settings.OPENAI_MODEL

            # Step 3a: Structure Analysis (streaming)
            step3a = ReasoningStep(
                "Analyzing Document Structure",
                "GPT-5 is identifying sections, headers, and overall document layout...",
                "search"
            )
            yield await send_reasoning_step(step3a)
            try:
                structure_prompt = (
                            ...
                            for item in chunk.output:
                                if getattr(item, 'type', '') == 'message' and getattr(item, 'content', None):
                                    for c in item.content:
                                        if getattr(c, 'type', '') == 'output_text' and getattr(c, 'text', None):
                                            structure_content += c.text
                                            step3a.content = f"Document structure: {structure_content[:100]}..."
                                            yield await send_reasoning_step(step3a)
                                            await asyncio.sleep(0.1)
                else:
                    if structure_response.output:
                ...
                step3a.complete()
                step3a.content = f"Document structure analyzed: {len(structure_content)} characters of insights"
                step3a.details = structure_content
                yield await send_reasoning_step(step3a)
            except Exception as e:
                step3a.error(f"Structure analysis failed: {str(e)}")
                yield await send_reasoning_step(step3a)
                structure_content = "Document structure analysis unavailable, proceeding with direct extraction."

            # Step 3b: Category Identification (streaming)
            step3b = ReasoningStep(
                "Identifying Risk Categories",
                "GPT-5 is scanning for regulatory risk categories and examination domains...",
                "search"
            )
            yield await send_reasoning_step(step3b)
            try:
                category_prompt = (
                    "Based on this First Day Letter, identify the main regulatory risk categories mentioned. "
                    "Look for terms like Credit Risk, Operational Risk, IT/Cybersecurity, BSA/AML, etc. "
                            ...
                            for item in chunk.output:
                                if getattr(item, 'type', '') == 'message' and getattr(item, 'content', None):
                                    for c in item.content:
                                        if getattr(c, 'type', '') == 'output_text' and getattr(c, 'text', None):
                                            category_content += c.text
                                            step3b.content = f"Found categories: {category_content[:100]}..."
                                            yield await send_reasoning_step(step3b)
                                            await asyncio.sleep(0.1)
                else:
                    if category_response.output:
                ...
                step3b.complete()
                step3b.content = f"Risk categories identified: {len(category_content)} characters of analysis"
                step3b.details = category_content
                yield await send_reasoning_step(step3b)
            except Exception as e:
                step3b.error(f"Category identification failed: {str(e)}")
                yield await send_reasoning_step(step3b)
                category_content = "Category analysis unavailable, using fallback classification."

            # Step 4: Final Extraction (non-streaming JSON)
            step4 = ReasoningStep(
                "Extracting Examination Requests",
                "GPT-5 is now extracting specific examination requests with all details...",
                "lightbulb"
            ...
            try:
                ...
                step4.content = "Performing final extraction with full context and reasoning..."
                yield await send_reasoning_step(step4)

                resp = client.responses.create(**extraction_params)
                step4.complete()
                step4.content = "Request extraction completed successfully with contextual analysis"
                yield await send_reasoning_step(step4)
            except Exception as e:
                step4.error(f"Request extraction failed: {str(e)}")
                yield await send_reasoning_step(step4)
                yield await send_error(f"AI analysis failed: {str(e)}")
                return
            ...
            if not content:
                step5.error("No content received from AI analysis")
                yield await send_reasoning_step(step5)
                yield await send_error("AI analysis returned no content")
                return
            ...
            try:
                ...
                step5.complete()
                step5.content = f"Successfully extracted {len(rfis)} examination requests from AI analysis."
                step5.details = f"Raw AI response:\n{content[:500]}..." if len(content) > 500 else content
                yield await send_reasoning_step(step5)
            except Exception as e:
                step5.error(f"Failed to parse AI response: {str(e)}")
                yield await send_reasoning_step(step5)
                yield await send_error(f"Failed to parse AI response: {str(e)}")
                return
            ...
            step6.complete()
            step6.content = "Category normalization completed."
            yield await send_reasoning_step(step6)
            ...
            try:
                ...
                step7.complete()
                step7.content = f"Successfully stored {len(inserted)} examination requests in database."
                yield await send_reasoning_step(step7)
                ...
                step7.error(f"Database storage failed: {str(e)}")
                yield await send_reasoning_step(step7)
                yield await send_error(f"Database storage failed: {str(e)}")
                return
